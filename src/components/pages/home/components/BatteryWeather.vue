...
<template>
  <div class="flex items-center justify-evenly bg-card px-3 py-1 text-foreground">
    <div class="flex items-center space-x-2">
      <div class="relative h-[18px] w-[30px] rounded border-2 border-border-strong p-0.5">
        <div class="h-full w-[var(--battery-percent)] rounded-sm bg-primary" :style="`--battery-percent:${batteryPercentage}%`"></div>
        <div class="absolute -right-1 top-1/2 h-2 w-1 -translate-y-1/2 rounded-sm bg-border-strong"></div>
      </div>
    ...
    <div class="flex items-center space-x-1 text-sm">
      ...
      <span class="text-xs text-danger">{{ weatherDescription }}</span>
    </div>
